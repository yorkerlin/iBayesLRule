...
struct igamma_series_impl {
  ...
  EIGEN_DEVICE_FUNC
  static Scalar run(Scalar a, Scalar x) {
    if (a >1e6 && x>= 0.8f && mode==SAMPLE_DERIVATIVE){
/*
...
*/
		...
		if (alpha > 8.0f) {
			if (0.9f * alpha <= x && x <= 1.1f * alpha) {
				const Scalar numer_1 = 1 + 24 * alpha * (1 + 12 * alpha);
				const Scalar numer_2 = 1440 * (alpha * alpha) + 6 * x * (53 - 120 * x)
					- 65 * x * x / alpha + alpha * (107 + 3600 * x);
				const Scalar denom = 1244160 * (alpha * alpha) * (alpha * alpha);
				return (numer_1 * numer_2 / denom);
			}
			const Scalar denom = numext::sqrt(8 * alpha);
			const Scalar term2 = denom / (alpha - x);
			const Scalar term3 = numext::pow(x - alpha - alpha * numext::log(x / alpha), Scalar(-1.5));
			const Scalar term23 = (x < alpha) ? term2 - term3 : term2 + term3;
			const Scalar term1 = numext::log(x / alpha) * term23
				- numext::sqrt(2 / alpha) * (alpha + x) / ((alpha - x) * (alpha - x));
			const Scalar stirling = 1 + 1 / (12 * alpha) * (1 + 1 / (24 * alpha));
			const Scalar numer = x * term1;
