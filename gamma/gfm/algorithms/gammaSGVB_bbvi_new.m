...
function [a,b,ll, settings]=gammaSGVB_bbvi_new( logL, D, settings, myCallback )
...
offset = 1e-2;
r=[log(exp(eta1-offset)-1.); log(exp(eta2)-1.)];
...
for sampleIndex=1:(settings.samples+1)
    ...
    fprintf('%d) nlz=%.4f post-test=%.4f e-test=%.4f adam-new\n',sampleIndex,logl,test,test2);
   ...
   eta1=log1pe(r(1:D))+offset;
   eta2=log1pe(r((D+1):end));
