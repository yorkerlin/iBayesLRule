...
function [a,b,ll,settings]=gammaSGVB_cvi_new( logL, D, settings, myCallback )
...
for sampleIndex=1:(settings.samples+1)
    ...
    fprintf('%d) nlz=%.4f post-test=%.4f e-test=%.4f cvi-new\n',sampleIndex,logl,test,test2);
    ...
    %line search
    index=deta1<0.;
    if length( eta1(index) )>0
        %stepSize(index)=min(  decay.*(eta1(index)-offset)./ (-deta1(index)), stepSize(index) );
        stepSize=min(min(  decay.*(eta1(index)-offset)./ (-deta1(index))), stepSize );
    end

    index=deta2<0.;
    if length( eta2(index) )>0
        %stepSize(index)=min(  decay.*eta2(index)./ (-deta2(index)), stepSize(index) );
        stepSize=min(min(  decay.*eta2(index)./ (-deta2(index))), stepSize );
    end

   eta1 = eta1 + stepSize .* deta1;
   eta2 = eta2 + stepSize .* deta2;
