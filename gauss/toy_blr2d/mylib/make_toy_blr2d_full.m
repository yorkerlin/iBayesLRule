...
file_name = strcat('./toy_example_experiment_data_full.mat');
load(file_name)
[n,n]=size(w1);
t = maxIters;


method = {'iBayesLRule', 'VOGN'};
colors={'k','r','g','b','c','m','y',[0.8 0.8 1]};
...
for m = 1:length(method)
    plot(w_all(1,t,m), w_all(2,t,m), 'or', 'color', colors{m+1}, 'linewidth', 8, 'markerfacecolor', colors{m+1}, 'markersize', 5);
    h(m+1) = plot_gaussian_ellipsoid(w_all(:,t,m), Sigma_all(:,:,t,m), 1);
    set(h(m+1), 'color', colors{m+1}, 'linewidth', 12);
end

h(length(method)+2) = plot_gaussian_ellipsoid(w_exact_vi, C_exact_vi, 1);
set(h(length(method)+2), 'color', colors{length(method)+2}, 'linestyle', '-.', 'linewidth', 8);
% full-exact
axis([0 20 0 12]);
plot(w_exact_vi(1), w_exact_vi(2), 'o', 'color', colors{length(method)+2}, 'linewidth', 6, 'markerfacecolor', colors{length(method)+2}, 'markersize', 5);



hl = legend(h, {'MAP', 'iBayesLRule-full',  'VOGN-full', 'Full-exact'}, 'location', 'northwest');
hx = xlabel('w_1');
hy = ylabel('w_2');
set(gca, 'fontsize', 24);
...
export_fig('./toy_blr2d_full.pdf', '-transparent')
%print(f, './toy_blr2d_full.pdf', '-dpdf', '-fillpage')
