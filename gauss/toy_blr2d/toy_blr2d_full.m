...
% #####################################################
% ############# Compute full-Exact Solution ###########
% #####################################################

% Compute the full-exact solution.
D = 2;
...
optMinFunc = struct('display', 1, 'Method', 'lbfgs', 'DerivativeCheck', 'off','LS', 2, 'MaxIter', 2000, 'MaxFunEvals', 2000, 'TolFun', 1e-6, 'TolX', 1e-6);
[v, f, exitflag, inform] = minFunc(funObj, v0, optMinFunc, t, X, gamma);
w_exact_vi = v(1:D);
U = triu(unpackcovariance(v(D+1:end),D));
C_exact_vi = U'*U;
...
for m = 1:length(method)
   ...
   for t = 1:maxIters
     ...
     case {'Vadam'}
        ...
        S_corrected = S;
        w = w - ss*( ( (sqrtm(S_corrected) + eye(D)./alpha)./ (1 - (gamma)^t) ) \ grad_average_corrected);
